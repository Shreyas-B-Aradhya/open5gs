        ...
        ogs_gtp2_create_bearer_request_t *req)
{
    ...
    if (!bearer) {
         ...
         * ALLOC-FAIL dump right above. Upstream asserts here; this fork
         * logs and drops the Create Bearer Request instead, so one UE's
         * EBI exhaustion cannot crash the whole MME.
         */
        ogs_error("[EBI-TRACK] CREATE-BEARER-REQUEST FAILED: "
                "EBI pool exhausted, dropping request (see dump above) "
